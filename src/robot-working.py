...
class Robot:
    def __init__(self):
        ...
        self.status_light = LED(26)
        self.status_light_state = 0
        
        self.script_light = LED(16)
        self.script_light_state = 0
    ...
    async def start(self):
        ...
        async for raw_frame in self.utils.camera.main():
            ...
            if self.ticks <= 15:
                self.script_light.on()
                self.script_light_state = 1
            ...
            if self.utils.switch_left.is_pressed:
                mode = Mode.Update
                if self.status_light_state == 0:
                    self.status_light.on()
                    self.status_light_state = 1
                await self.update()
                ...
                if self.ticks % 4 == 0:
                    self.status_light_state = not self.status_light_state
                if self.status_light_state == 0:
                    self.status_light.off()
                elif self.status_light_state == 1:
                    self.status_light.on()
